...
const WEAPON_LOADOUTS = {
  spear: { label: 'Spear', description: 'A reinforced ash spear with a longer reach, a heavier thrust, and a bright steel leaf at the point.', range: 5.1, damage: 68, aim: .23, pitch: .22, hitAt: .38, knockback: .28, duration: .54, cooldown: .08 },
  crossbow: { label: 'Crossbow', description: 'A compact ranged option: slower to fire, but precise across an open chamber.', range: 8.5, damage: 76, aim: .12, pitch: .14, hitAt: .58, knockback: .08, duration: .72, cooldown: .24 },
  wand: { label: 'Ember Wand', description: 'A rune-carved wand that launches area fireballs. Its crystal changes color with the latest spell learned.', range: 11, damage: 92, aim: .2, pitch: .26, hitAt: .48, knockback: .1, duration: .64, cooldown: .28, projectileType: 'wand-fireball' },
};
...
const MUSIC_PATTERNS = {
  dungeon: {
    tempo: 58,
    melody: [196, 0, 233, 0, 175, 0, 156, 0, 196, 0, 262, 0, 233, 0, 175, 0],
    bass: [49, 49, 55, 55, 46, 46, 41, 41, 49, 49, 55, 55, 39, 39, 46, 46],
    drone: [49, 55, 46, 41],
    melodyType: 'triangle',
    bassType: 'sine',
    noteLength: 1.35,
  ...
  boss: {
    tempo: 74,
    melody: [147, 0, 175, 0, 196, 175, 156, 0, 147, 0, 131, 0, 156, 131, 147, 0],
    bass: [37, 37, 44, 44, 41, 41, 34, 34, 37, 37, 44, 44, 31, 31, 34, 34],
    drone: [37, 41, 34, 31],
    melodyType: 'square',
    bassType: 'triangle',
    noteLength: .92,
...
function beginLaunchTransition() {
  state.launchTransition = { elapsed: 0, duration: 1.85 };
  state.keys.clear();
...
function updateLaunchTransition(delta) {
  ...
  if (state.launchTransition.elapsed >= state.launchTransition.duration) state.launchTransition = null;
}
function drawLaunchTransition(now) {
  ...
  ctx.save();
  ctx.fillStyle = `rgba(5, 3, 2, ${.94 * veil + .025})`;
  ctx.fillRect(0, 0, width, height);
  ctx.globalAlpha = veil * .22;
  ctx.fillStyle = '#d7bd87';
  for (let y = 0; y < height; y += 7) ctx.fillRect(0, y, width, 1);
  if (veil > .05) {
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.globalAlpha = veil * .88;
    ctx.fillStyle = '#e7d3a7';
    ctx.shadowBlur = 18;
    ctx.shadowColor = '#9e7542';
    ctx.font = `bold ${Math.max(18, height * .042)}px Georgia`;
    ctx.fillText('THE OPERATIONS DUNGEON', width / 2, height * .44);
    ctx.shadowBlur = 0;
    ctx.fillStyle = '#b9a57e';
    ctx.font = `${Math.max(10, height * .017)}px Georgia`;
    ctx.fillText('FLOOR 01 // THE ENTRANCE HALL', width / 2, height * .54);
    ctx.globalAlpha = veil * .45;
    ctx.strokeStyle = '#b98542';
    ctx.lineWidth = 1;
    ctx.beginPath(); ctx.moveTo(width * .31, height * .63); ctx.lineTo(width * .69, height * .63); ctx.stroke();
  }
  ctx.globalAlpha = .12 + veil * .24;
  ctx.fillStyle = '#d99762';
  for (let index = 0; index < 9; index += 1) {
    const seed = index * 83.7 + Math.floor(now / 90);
    const x = fract(Math.sin(seed) * 43758.5) * width;
    const y = fract(Math.sin(seed * 1.6) * 27183.2) * height;
    ctx.fillRect(x, y, 8 + fract(Math.sin(seed * 2.4) * 9123) * width * .08, 1 + index % 2);
  }
  ctx.restore();
}
...
function wandColorForSpell() { return state.lastSpell?.color || '#d96642'; }
...
function drawSpear(now) {
  ...
  const bob = moving ? Math.sin(state.weapon.bobPhase) * .026 : Math.sin(now / 700) * .007;
  let thrust = 0;
  let sideOffset = 0;
  let roll = .1;
  if (active && t < .18) {
    const windup = easeOutCubic(t / .18);
    thrust = -.3 * windup;
    sideOffset = .1 * windup;
    roll = .1 - .25 * windup;
  } else if (active && t < .54) {
    const drive = easeOutCubic((t - .18) / .36);
    thrust = -.3 + 1.14 * drive;
    sideOffset = .1 - .18 * drive;
    roll = -.15 + .16 * drive;
  } else if (active) {
    const recovery = easeOutCubic((t - .54) / .46);
    thrust = .84 - .84 * recovery;
    sideOffset = -.08 + .08 * recovery;
    roll = .01 + .09 * recovery;
  }
  const faces = [];
  const scale = .78;
  const origin = { side: .36 + sideOffset, forward: 1.48 + thrust, z: -.04 + bob };
  addWeaponBox(faces, origin, { side: 0, forward: 0, z: .68 * scale }, [.09 * scale, .085 * scale, 1.48 * scale], roll, '#704622', .98, 'wood');
  addWeaponBox(faces, origin, { side: 0, forward: .01, z: .08 * scale }, [.16 * scale, .13 * scale, .16 * scale], roll, '#5b351f', .92, 'leather');
  addWeaponBox(faces, origin, { side: 0, forward: .05, z: 1.12 * scale }, [.25 * scale, .14 * scale, .1 * scale], roll, '#8d673b', .92, 'wood');
  addWeaponBox(faces, origin, { side: 0, forward: .09, z: 1.39 * scale }, [.2 * scale, .12 * scale, .3 * scale], roll, '#b9b7a0', 1.02, 'steel');
  addWeaponBox(faces, origin, { side: -.055, forward: .12, z: 1.51 * scale }, [.055 * scale, .13 * scale, .2 * scale], roll, '#f1e7c2', 1.08, 'steel');
  addWeaponBox(faces, origin, { side: .055, forward: .12, z: 1.51 * scale }, [.055 * scale, .13 * scale, .2 * scale], roll, '#8b8d89', .9, 'steel');
  addWeaponBox(faces, origin, { side: 0, forward: .14, z: 1.72 * scale }, [.09 * scale, .1 * scale, .23 * scale], roll, '#f5edca', 1.08, 'steel');
  renderFaces(faces, 1, true);
  if (active && t > .24 && t < .75) {
    const tip = projectCameraPoint({ side: origin.side, forward: origin.forward + .14, z: origin.z + 1.84 * scale });
    if (tip) {
      const flare = Math.sin((t - .24) / .51 * Math.PI);
      ctx.save();
      ctx.globalAlpha = flare * .8;
      ctx.fillStyle = '#f5e7ad';
      ctx.shadowBlur = 19;
      ctx.shadowColor = '#d59643';
      ctx.beginPath();
      ctx.arc(tip.x, tip.y, Math.max(2, canvas.height * .011), 0, TAU);
      ctx.fill();
      ctx.restore();
    }
  }
...
function spawnWandFireball() {
  const direction = playerAimDirection();
  const color = wandColorForSpell();
  const spellBonus = state.lastSpell?.kind === 'fireball' ? 14 : 0;
...
function updateBoss(delta) {
  ...
  if (nextPhase !== boss.phase) {
    ...
    boss.shield = nextPhase === 2 ? 170 : nextPhase === 3 ? 230 : 0;
    boss.cooldown = .35;
    state.shakeTime = .8;
  ...
  if (pattern === 0) {
    ...
    boss.shield = Math.max(boss.shield, boss.phase === 3 ? 100 : 78);
...
function updatePrompt(delta = 0) {
  ...
  const nextKey = doorDistance < 1.65 ? 'E' : target ? '✦' : item ? 'E' : state.lastSpell ? 'Q' : 'WASD';
